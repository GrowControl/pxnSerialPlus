...
if [[ ! -z $MANU_NAME ]]; then
	UDEV_RULE="SUBSYSTEM==\"tty\", ATTRS{idVendor}==\"$VENDOR_ID\", ATTRS{idProduct}==\"$PRODUCT_ID\", SYMLINK+=\"tty${MANU_NAME}%s{devpath}\""
fi
...
if [[ ! -z $DO_INSTALL ]]; then
	...
	echo "$UDEV_RULE" >> /etc/udev/rules.d/99-usb-serial.rules || exit 1
	echo
